    ...
    <div>
      ...
      <div className="kpi-grid" style={{ gridTemplateColumns: "repeat(4,1fr)", marginBottom: 24 }}>
        ...
        <div className="kpi-card purple">
          ...
          <div className="kpi-value" style={{ fontSize: 22 }}>
            {paid.length
              ? (paid.reduce((s, i) => s + Math.floor((new Date(i.paymentDate) - new Date(i.dateOfReceipt)) / 86400000), 0) / paid.length).toFixed(0) + "d"
              : "—"}
          </div>
      ...
      {readyToPay.length > 0 && (
        <div className="alert alert-info" style={{ marginBottom: 16 }}>
          💳 {readyToPay.length} invoice(s) approved and ready for payment totalling {fmt(totalPending)}.
        </div>
      )}

      {/* Tabs */}
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        <button onClick={() => { setFilter("pending"); resetPage(); }} className={`btn btn-sm ${filter === "pending" ? "btn-primary" : "btn-outline"}`}>
          Pending Payment ({readyToPay.length})
        </button>
        <button onClick={() => { setFilter("paid"); resetPage(); }} className={`btn btn-sm ${filter === "paid" ? "btn-primary" : "btn-outline"}`}>
      ...
      <div className="filter-bar">
        <SearchBar value={search} onChange={handleFilterChange(setSearch)} placeholder="Search by vendor or invoice ID..." />
        <FilterSelect value={modeFilter}     onChange={handleFilterChange(setModeFilter)}     options={PAYMENT_MODE_OPTIONS} placeholder="All Payment Modes" />
        ...
        <span style={{ fontSize: 12, color: "var(--text-muted)", whiteSpace: "nowrap" }}>{filtered.length} records</span>
      </div>
          ...
          <table>
            <thead>
              <tr>
                ...
                {filter === "paid" && <><th>Payment Date</th><th>Bank Ref</th><th>Status</th></>}
